...
protected:
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		...
		if (m_nVisitedCells < m_nMazeWidth * m_nMazeHeight)
		{
			...
			if (m_stack.top().first==1 && m_stack.top().second == 15) {
				ofstream output_mto_foda("output2.txt");
				
				for (int i = 0; i < m_nMazeHeight * m_nMazeWidth;i++) {
					if (i!=0 && i % 40 == 0) {
						if ((m_maze[i] - 10) < 10) {
							output_mto_foda << '0' << m_maze[i] - 10 << "//" << '-' << (i / 40)-1 << '-' << '\n';
						}
						else
							output_mto_foda << m_maze[i] - 10 << "//" << '-' << (i / 40)-1 << '-' << '\n';
						
					}
					else {
						if ((m_maze[i]-10)<10) {
							output_mto_foda <<'0'<< m_maze[i] - 10 << "//";
						}
						else
							output_mto_foda << m_maze[i] - 10 << "//";
					}
					
				}
				output_mto_foda.close();
